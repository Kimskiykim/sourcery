...
- получать bootstrap entrypoints (`AGENTS.md`, `README.md`, `CLAUDE.md`, `agents_md/FRAMEWORK.md`, и похожие стартовые документы) отдельно от поисковых совпадений
...
3. Сначала дочитать `bootstrapNotes` из context pack через `notes.read`, затем самые релевантные поисковые `notes`.
